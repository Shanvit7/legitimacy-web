...
export const getValidNumber = (value: string, { max, min = 0, loop = false }: { max: number, min?: number, loop?: boolean }) => {
...
export const getValidHour = (value: string) => isValidHour(value) ? value : getValidNumber(value, { max: 23 });
export const getValid12Hour = (value: string) => isValid12Hour(value) ? value : getValidNumber(value, { min: 1, max: 12 });
export const getValidMinuteOrSecond = (value: string) => isValidMinuteOrSecond(value) ? value : getValidNumber(value, { max: 59 });

// Arrow step increment handlers
export const getValidArrowNumber = (value: string, { min, max, step }: { min: number, max: number, step: number }) => {
  let numericValue = parseInt(value, 10);
  if (!isNaN(numericValue)) {
    numericValue += step;
...
export const getValidArrowHour = (value: string, step: number) => getValidArrowNumber(value, { min: 0, max: 23, step });
export const getValidArrow12Hour = (value: string, step: number) => getValidArrowNumber(value, { min: 1, max: 12, step });
export const getValidArrowMinuteOrSecond = (value: string, step: number) => getValidArrowNumber(value, { min: 0, max: 59, step });
...
export const setDateByType = (date: Date, value: string, type: TimePickerType, period?: Period) => {
  switch (type) {
    ...
    case '12hours': return period ? set12Hours(date, value, period) : date;
    default: return date;
  }
};
...
export const getArrowByType = (value: string, step: number, type: TimePickerType) => {
  switch (type) {
    ...
    case '12hours': return getValidArrow12Hour(value, step);
    default: return '00';
  }
};
